...
hpo = "/hpf/largeprojects/ccmbio/dennis.kao/gene_data/HPO/166_HPO.txt" 
...
for index, sample in enumerate(vcf_dict["samples"]):
    vcf_dict["{sample}_GENOTYPE"] = gts[str(index)]
...
for index, sample in enumerate(vcf_dict["samples"]):
    vcf_dict["{sample}_AD"] = ads[str(index)]
...
for index, sample in enumerate(vcf_dict["samples"]):
    vcf_dict["{sample}_DP"] = dps[str(index)]
